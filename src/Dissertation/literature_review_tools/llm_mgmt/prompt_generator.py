...
methods: An array of research methods, approaches, or methodologies used.
findings: Key results, conclusions, or outcomes.
industries: Identify the relevant SIC industry classification. Select the best match from: {IndustryTaxonomy.industry_taxonomy_sci_prompt['prompt']}.
...
1. Relational Mapping: If the abstract discusses multiple activities, create multiple objects in the activities array. Ensure knowledge_flows, tacit_knowledge_types, and technologies are nested inside the specific activity they relate to.
2. Conceptual Alignment: When academic constructs are not named verbatim but are strongly implied by described mechanisms, map them to the closest preferred value using domain reasoning.
3. Conciseness: Use professional academic wording and ensure lists contain unique values.
4. Null Values: For all fields except the 'relevance' field, if a field is not mentioned, return an empty string ("") or empty array ([]) as appropriate."""
}
