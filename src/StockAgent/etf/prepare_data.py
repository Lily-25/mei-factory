...
class DataFactory(SchemaManager):
    def __init__(self):
        super().__init__()
        self.price_and_fund_merger_dir = self.dir + 'etf_etl_data/price_and_fund_merger/'
        self.refresh_config()

    def refresh_config(self):
        self.schema_config_mgt.insert('etf.price_and_fund_merger_dir',
    ...
    def align_price_and_fund_flow(self, history_price_file):
        ...
        if (not os.path.exists(history_price_file)
                or not check_whether_files_created_today(history_price_file,
                                                         self.schema_tmp_config['etf']['refresh_time'])):
            return

        filename = os.path.basename(history_price_file)

        if (os.path.splitext(filename)[0]
                not in self.etf_current_observation_pool_dict.keys()):
            return

        history_fund_flow_file = (self.schema_config['etf']['hist_fund_flow_dir']
                                         + filename)
        ...
        if (('Close_df1' in merged_df.columns)
                and (merged_df[merged_df['Close'] != merged_df['Close_df1']].shape[0])):
            print(f'{filename}数据冲突')
            return
    ...
    def batch_align_price_and_fund_flow(self):
        ...
        walk_directory(self.schema_config['etf']['hist_price_dir'], self.align_price_and_fund_flow)


if __name__ == '__main__':
    etf_obj = DataFactory()
    etf_obj.batch_align_price_and_fund_flow()